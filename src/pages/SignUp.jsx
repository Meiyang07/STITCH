    ...
    <section className="min-h-screen bg-[#FAF8F3] pt-[76px]">
      <div className="grid min-h-[calc(100vh-76px)] lg:grid-cols-[.9fr_1.1fr]">
        <div className="relative hidden overflow-hidden bg-charcoal lg:block">
          ...
          <div className="absolute inset-x-0 bottom-0 p-12 text-white xl:p-16"><p className="eyebrow text-[#d3b783]">CREATE YOUR PROFILE</p><p className="mt-5 max-w-lg font-serif text-5xl leading-[.98]">One account for fit, orders and saved styles.</p></div>
        </div>
        ...
        <div className="flex items-center px-5 py-14 sm:px-10 lg:px-16 xl:px-24">
          <div className="mx-auto w-full max-w-2xl">
            ...
            <p className="mt-5 text-sm leading-7 text-muted">Create a customer account to checkout ready-made pieces, save measurements and keep your wishlist together.</p>
            ...
            <form onSubmit={submit} className="mt-9 grid gap-5 border-t border-black/15 pt-8 sm:grid-cols-2">
              ...
              <label className="block"><span className="mb-2 flex items-center gap-2 text-[10px] uppercase tracking-[.16em] text-muted"><Phone size={14}/> Phone · 10 digits</span><input required inputMode="numeric" pattern="[0-9]{10}" maxLength="10" value={form.phone} onChange={(e) => setForm({ ...form, phone: digitsOnly(e.target.value) })} placeholder="98XXXXXXXX" /></label>
            ...
            <p className="mt-5 text-[11px] leading-5 text-muted/70">Current project note: accounts are stored locally on this browser. Production authentication needs a secure backend.</p>
          </div>
        </div>
      </div>
    </section>
