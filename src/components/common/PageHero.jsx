...
export default function PageHero({ eyebrow, title, copy, image, align = 'left', compact = false }) {
  return (
    <section className={`relative overflow-hidden bg-charcoal ${compact ? 'min-h-[60vh]' : 'min-h-[76vh]'} flex items-end`}>
      {image ? (
        <motion.img initial={{ scale: 1.05 }} animate={{ scale: 1 }} transition={{ duration: 1.3 }} src={image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-60" />
      ) : (
        <div className="absolute inset-0 flex items-center justify-center bg-[#292929] text-white/20">
          <div className="text-center">
            <ImageIcon className="mx-auto" size={28} strokeWidth={1.1} />
            <p className="mt-3 text-[9px] uppercase tracking-[.22em]">Page hero image</p>
          </div>
        </div>
      )}
