...
export default function Button({ children, to, href, variant = 'dark', className = '', arrow = false, ...props }) {
  const cls = `focus-lux inline-flex min-h-12 items-center justify-center gap-3 border px-5 sm:px-6 text-[11px] font-medium uppercase tracking-[0.18em] transition-colors duration-300 ${styles[variant]} ${className}`;
  const content = <>{children}{arrow && <ArrowUpRight size={15} strokeWidth={1.6} />}</>;
