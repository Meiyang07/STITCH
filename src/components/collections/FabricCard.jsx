    ...
    <article className="group">
      <div className="relative aspect-square overflow-hidden bg-warm">
        <button onClick={() => onOpen(fabric)} className="h-full w-full text-left" aria-label={`View ${fabric.name} details`}>
          {fabric.image ? (
            <img src={fabric.image} alt={`${fabric.name} fabric texture`} loading="lazy" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.04]" />
          ) : (
            <ImageSlot label={`${fabric.name} fabric`} />
          )}
        </button>
