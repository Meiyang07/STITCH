  ...
  return (
    <motion.article initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .15 }} transition={{ duration: .55 }} className={`group ${className}`}>
      <div className={`relative overflow-hidden bg-warm ${editorial ? 'aspect-[4/5]' : 'aspect-[4/5]'}`}>
        <Link to={path} aria-label={`Explore ${item.name}`} className="block h-full w-full">
          {item.image ? (
            <img src={item.image} alt={item.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.035]" />
          ) : (
            <ImageSlot label={`${item.name} image`} />
          )}
        </Link>
